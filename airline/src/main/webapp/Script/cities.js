...
var deptCd;
function append_city(id) {
	deptCd = id;
	var x = $('#'+deptCd).offset().top;
	var y = $('#'+deptCd).offset().left;
...
function hide_div(e) {
	...
	if (classname.indexOf('click') > '-1' || $(e.target)[0].id == deptCd)
		return;

	if ($('#'+deptCd).val() == '') {
		$('#'+deptCd).val('请输入城市中文或拼音');
		$('#'+deptCd).css('color', '#B7B7B7');
	}
	$("#cityarea").hide();
}
